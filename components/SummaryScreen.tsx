...
const SummaryScreen: React.FC<Props> = ({ gameState, onContinue, onAdjustAsset, onSellCar }) => {
        ...
        <div className="flex space-x-3">
            <button 
                ...
                className={`flex-1 py-3 rounded font-bold text-sm transition-colors flex items-center justify-center gap-1 ${
                    canSell 
                    ? 'bg-zinc-700 hover:bg-zinc-600 text-emerald-400 border border-zinc-600' 
  ...
  return (
    <div className="flex flex-col h-full bg-zinc-900 p-6 overflow-y-auto custom-scrollbar">
      <h1 className="text-3xl font-bold text-white mb-2 cyber-font border-l-4 border-emerald-500 pl-4">
        STAGE REPORT <span className="text-emerald-500 text-lg">//{gameState.stage}</span>
      </h1>
      ...
      <div className="bg-black border border-zinc-800 p-4 rounded-lg mb-6 shadow-[0_0_10px_rgba(0,255,100,0.1)]">
        <div className="flex justify-between items-end mb-2">
            ...
            <span className={`font-mono font-bold ${totalNetWorth >= THRESHOLDS.SUCCESS_CASH ? 'text-emerald-400' : 'text-zinc-300'}`}>
      ...
      <div className="bg-zinc-950 rounded-lg p-1 mb-6 border border-zinc-800 relative">
        ...
        <div className="p-5">
            <h2 className="text-emerald-500 font-bold tracking-widest text-sm mb-4 flex items-center">
            ...
            <div className="space-y-2 font-mono text-sm">
                ...
                <div className="flex justify-between items-center py-1 border-b border-zinc-800/50">
                    ...
                    <div className="text-right">
                        <span className={investmentData.total >= 0 ? 'text-emerald-400' : 'text-red-500'}>
                ...
                <div className="border-t border-zinc-700 pt-3 mt-2 flex justify-between items-center">
                    ...
                    <span className={`text-xl font-bold ${netChange >= 0 ? 'text-emerald-400' : 'text-red-500'}`}>
      ...
      <div className="mb-8 flex justify-between items-center bg-zinc-800 p-4 rounded border border-zinc-700">
        <div>
            ...
            <div className={`font-mono text-3xl font-bold ${gameState.stats.cash < 0 ? 'text-red-500 neon-text' : 'text-emerald-400 neon-text'}`}>
      ...
      <div className="grid grid-cols-3 gap-2 mb-8">
          ...
          >
              ...
              <div className="text-[10px] text-emerald-400 font-mono">¥0/mo</div>
              <div className="text-[10px] text-red-400 mt-1">HP/PSY 大幅持续受损</div>
          </button>
          <button
             onClick={() => setSelectedRentType('cheap')}
             className={`p-3 rounded border text-left transition-all ${selectedRentType === 'cheap' ? 'bg-emerald-900/30 border-emerald-500' : 'bg-zinc-800 border-zinc-700 opacity-60'}`}
          >
              <div className="text-xs font-bold text-white mb-1">🏠 胶囊笼屋</div>
              <div className="text-[10px] text-emerald-400 font-mono">¥{COSTS.RENT_CHEAP}/mo</div>
              <div className="text-[10px] text-zinc-400 mt-1">低维护，环境恶劣</div>
          </button>
          <button
             onClick={() => setSelectedRentType('expensive')}
             className={`p-3 rounded border text-left transition-all ${selectedRentType === 'expensive' ? 'bg-emerald-900/30 border-emerald-500' : 'bg-zinc-800 border-zinc-700 opacity-60'}`}
          >
              <div className="text-xs font-bold text-white mb-1">🏢 智能公寓</div>
              <div className="text-[10px] text-emerald-400 font-mono">¥{COSTS.RENT_EXPENSIVE}/mo</div>
        ...
        {gameState.config.carType !== 'none' && (
             <div className="bg-zinc-800 p-4 rounded-lg border border-zinc-700">
                ...
                >
                    ...
                    <span className="text-emerald-500 font-mono font-bold">+ ¥{(gameState.config.carType === 'gas' ? COSTS.CAR_GAS_PRICE : COSTS.CAR_ELEC_PRICE) * 0.6}</span>
                </button>
            </div>
        )}
      </div>
      ...
      <button
        ...
        className="mt-auto w-full bg-gradient-to-r from-emerald-800 to-emerald-600 hover:from-emerald-700 hover:to-emerald-500 text-white font-bold py-4 rounded-lg shadow-[0_0_20px_rgba(4,120,87,0.4)] transition-all border-t border-emerald-400/30 uppercase tracking-widest"
      >
        INITIATE NEXT CYCLE
      </button>
    </div>
